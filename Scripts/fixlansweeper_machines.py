...
machines = open("machines_clean.csv", 'r')

read_lines = read_file.readlines()
machine_lines = machines.readlines()
...
for i in range(len(read_lines)):
    ...
    skip = False
    for j in range(len(machine_lines)):
        if read_words[1] in machine_lines[j] and "CS" in read_words[8].split(" - ")[1]: 
            #if the ip is already in the machines.csv file, we know its a duplicate machine
            #and then we double check to make sure its a CS machine
            skip = True
            break
    if not skip:
